...
@Component({
  selector: 'app-categoria-muestra-detalle',
  templateUrl: './categoria-muestra-detalle.component.html',
  styleUrls: ['./categoria-muestra-detalle.component.scss']
})
export class CategoriaMuestraDetalleComponent implements OnInit {
  ...
  constructor(private categoriaService: CategoriaMuestraService, private fb: FormBuilder,
  ...
  ngOnInit() {
    console.log('Navigate to categoria muestra detail ', this.categoriaId);
  }
      ...
      this.categoriaService.createItem({'id': this.categoriaForm.value.id,
        'descripcion': this.categoriaForm.value.descripcion }).subscribe(
          response => {
            ...
            this.router.navigate(['/categoriasMuestra']);
