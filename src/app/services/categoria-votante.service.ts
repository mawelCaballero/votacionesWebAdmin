...
export class CategoriaVotanteService implements CRUDaction<Categoria> {
  ...
  constructor(private http: Http) {
    ...
    return this.http.get(config.url + 'categoriaVotante', { headers : this.getHeaders() }).map(
      response => {
  ...
  getItemById(_key: string ): Observable<Categoria> {
    return this.http.get(config.url + 'categoriaVotante/' + _key, {headers : this.getHeaders() }).map(
        response => {
  ...
  updateItem(params: any) {
    return this.http.put(config.url + 'categoriaVotante/' + params['_key'] ,
  ...
  createItem(params: any): Observable<Categoria> {
    return this.http.post(config.url + 'categoriaVotante' , params ,
    {headers : this.getHeaders() } ).map(response => {
      const categoria = JSON.parse(response['_body']);
          return <Categoria>categoria;
    });
  ...
  deleteItem(_key: string): Observable<Categoria[]> {
    return this.http.delete(config.url + 'categoriaVotante/' + _key,
    {headers : this.getHeaders() } ).map(response => {
        return null;
    });
  }
